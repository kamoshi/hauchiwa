...
fn main() {
    ...
    let posts_handle = loader::glob_assets(
        &mut config,
        "examples/simple_ssg_data/posts/**/*.md",
        |_, file| {
            let content = String::from_utf8(file.metadata).unwrap();
            let title = content
                .lines()
                .next()
                .unwrap_or("Untitled")
    ...
    config.add_task(
        (posts_handle,),
        |_, (posts,): (&loader::Registry<(Utf8PathBuf, Post)>,)| {
            posts
                .values()
                .map(|(_, post)| Page {
                    url: post.url.clone(),
    ...
    config.add_task(
        ...
        |_, (posts,): (&loader::Registry<(Utf8PathBuf, Post)>,)| {
            let post_links = posts
                .values()
                .map(|(_, post)| format!("<li><a href=\"{}\">{}</a></li>", post.url, post.title))
                .collect::<String>();
