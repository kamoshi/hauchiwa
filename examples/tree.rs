...
fn main() {
    ...
    let root_task = site_config.add_task((), |_| {
        println!("Executing Root Task");
        "Hello from the root!".to_string()
    });

    let child_task_1 = site_config.add_task((root_task,), |(root_message,)| {
        println!("Executing Child Task 1");
        format!("{} | Child 1 processed it.", root_message)
    });

    let child_task_2 = site_config.add_task((root_task,), |(root_message,)| {
        println!("Executing Child Task 2");
        root_message.len()
    });

    let grandchild_task =
        site_config.add_task((child_task_1, child_task_2), |(msg, len)| {
    ...
    let independent_task = site_config.add_task((), |_| {
    ...
    println!("\n--- Running Tasks ---");
    let results_cache = executor::run_once(&mut site);
    println!("--- Finished ---\n");
