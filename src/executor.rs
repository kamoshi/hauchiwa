...
fn run_tasks_parallel<G: Send + Sync>(
    ...
    let (task_sender, task_receiver) = unbounded::<(NodeIndex, Arc<dyn Task<G>>, Vec<Dynamic>)>();
    ...
    std::thread::scope(|s| {
        ...
        for &node_index in nodes_to_run {
            if dependency_counts.get(&node_index).cloned().unwrap_or(0) == 0 {
                ...
                task_sender.send((node_index, task, dependencies)).unwrap();
            }
        }
...
pub fn watch<G: Send + Sync>(site: &mut Site<G>, data: G) {
